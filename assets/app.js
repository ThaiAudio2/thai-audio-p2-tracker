...
function dayStatusOfNode(node, schedOwner, day) {
  ...
  // A freshly added sub-task (noAuto) stays blank until it is given a status of
  // its own — adding a sub-task must not inherit the parent's painted days.
  // Only an explicit per-day override (handled above) shows. Existing sub-tasks
  // without this flag keep auto-painting the schedule as before.
  if (node !== schedOwner && node.noAuto && (!node.status || node.status === 'NS')) return null;
...
function taskRowHTML(t) {
  ...
  if (isExp) {
    if (editing) {
      ...
      // Always show the sub-task list + "＋ Add sub-task" row, even for a task
      // that has none yet — so any heading can gain sub-tasks without Edit mode.
      html += subRowsHTML(t);
...
function wireBoard() {
  ...
  wrap.addEventListener('click', (e) => {
    ...
    if ((n = b('data-sadd'))) { const t = findTask(n.dataset.sadd).t; t.subtasks = t.subtasks || []; t.subtasks.push({ id: uid('s-'), title: 'New sub-task', status: 'NS', note: '', days: {}, noAuto: true }); commit(); refreshAll(); return; }
